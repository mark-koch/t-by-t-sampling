...
def iter_enumerate_combo_chunks(
    errs: np.ndarray, max_w: int
...
class DemErrorSet:
    ...
    def find_masks_reached_by_errors_up_to(
        self, *, max_distance: int, opposing_masks: set[int] | None = None
    ) -> set[int]:
        if max_distance > 3:
    ...
    def find_errors_for_midpoint_masks(
        self, midpoints: set[int], max_distance: int
    ) -> dict[int, set[frozenset[int, ...]]]:
        result: dict[int, set[frozenset[int, ...]]] = {}
        if len(self.masks.shape) == 1:
            for chunk, indices in iter_enumerate_combo_chunks(
                self.masks, max_w=max_distance
            ):
    ...
    def combine_midpoint_errors(
        self, midpoint_errors: dict[int, set[frozenset[int, ...]]]
    ) -> set[frozenset[int, ...]]:
        result = set()
    ...
    def find_logical_errors(self, max_distance: int) -> list[tuple[int, ...]]:
        ...
        potential_midpoints = self.find_masks_reached_by_errors_up_to(
            max_distance=store_w
        )
        actual_midpoints = self.find_masks_reached_by_errors_up_to(
            max_distance=search_w, opposing_masks=potential_midpoints
        )
        groups = self.find_errors_for_midpoint_masks(
            actual_midpoints, max(store_w, search_w)
        )
        logical_errors = self.combine_midpoint_errors(groups)
        logical_errors = [
            tuple(sorted(e)) for e in logical_errors if len(e) <= max_distance
        ]
        logical_errors = sorted(logical_errors, key=lambda e: (len(e), e))
        return logical_errors

    def expand_logical_errors(
        self, logical_errors: list[tuple[int, ...]]
    ) -> list["DemCombinedError"]:
        result = []
...
def analyze_solerr_discard_vs_error_rate(
    error_set: DemErrorSet, logical_errors: list["DemCombinedError"]
):
    e2f = {}
    for k_cond in range(len(error_set.errors)):
        ...
        for logical_err in logical_errors:
            p += math.prod(
                error_set.probs[k] for k in logical_err.src_errors if k_cond != k
            )
        e2f[k_cond] = p
    ...
    for k, v in sorted(e2f.items(), key=lambda e: (e[1], e[0])):
        ...
        p0b = chance_of_exactly_0(
            [error_set.probs[k] for k in allowed_singleton_errors]
        )
        p1b = chance_of_exactly_1(
            [error_set.probs[k] for k in allowed_singleton_errors]
        )
        ...
        for logical_err in logical_errors:
            for k_cond in logical_err.src_errors:
                if k_cond in allowed_singleton_errors:
                    fail_sets.add(
                        frozenset(e for e in logical_err.src_errors if e != k_cond)
                    )
                else:
