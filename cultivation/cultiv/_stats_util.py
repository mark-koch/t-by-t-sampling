...
def stat_to_gap_stats(
    ...
    return [
        gap_stat
        for stat in stats
        for gap_stat in _stat_to_gap_stats_single(stat, rounding=rounding, func=func)
    ]


def split_by_gap_threshold(
    stats: list[sinter.TaskStats], *, gap_rounding: int, keep_zero: bool = False
) -> list[sinter.TaskStats]:
    ...
    return [stat for stat in stats if keep_zero or stat.json_metadata.get("gap", 1) > 0]


def split_into_gap_distribution(
    stats: list[sinter.TaskStats], *, gap_rounding: int
) -> list[sinter.TaskStats]:
    stats = stat_to_gap_stats(
        stats,
        rounding=gap_rounding,
...
def split_by_custom_count(
    ...
    for stat in stats:
        for key, count in stat.custom_counts.items():
            ...
            result.append(
                stat.with_edits(
                    json_metadata={**stat.json_metadata, k: v, "hits": count},
                    strong_id=stat.strong_id + f"{k},{v}",
                )
            )
    return result
...
def _stat_to_gap_stats_single(
    ...
    anon_stat = sinter.AnonTaskStats(
    ...
    max_gap = max(
        int(k[1:])
        for k in anon_stat.custom_counts.keys()
        if k.startswith("C") or k.startswith("E")
    )
    for cor_gap, hits in anon_stat.custom_counts.items():
        ...
        gap = (
            max_gap
            if gap == max_gap
            else min(max_gap, round(gap / rounding) * rounding)
        )
        assert gap >= 0
    ...
    for gap in sorted(gap_stats.keys()):
        ...
        choice = func(
            GapArg(
                ...
                less=less,
                more=more,
                at_least=gap_stat + more,
                at_most=gap_stat + less,
            )
        )
        less += gap_stat
        result.append(
...
def preprocess_intercepted_simulation_stats(
    stats: list[sinter.TaskStats],
) -> list[sinter.TaskStats]:
    """
    ...
    for stat in stats:
        ...
        result.append(
            stat.with_edits(
                ...
                decoder=stat.decoder[:-2]
                .replace("vec_intercept", "Vector Sim")
                .replace("twirl_intercept", "Stabilizer Sim"),
            )
        )
