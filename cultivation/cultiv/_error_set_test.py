...
def test_iter_triplet_chunks():
    actual = []
    for chunk in iter_triplet_chunks(
        np.array(
            [
                0b000001,
                0b000110,
...
def test_find_errors():
    ...
    assert np.array_equal(
        ds.probs, [0.125, 0.375, 0.125, 0.125, 0.125, 0.25, 0.25, 0.25]
    )
    ...
    assert (
        ds.find_masks_reached_by_errors_up_to(max_distance=2, opposing_masks=stored2)
        == set()
    )
