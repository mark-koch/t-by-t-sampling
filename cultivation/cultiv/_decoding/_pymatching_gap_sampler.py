...
from latte.dem_util import dem_with_compressed_detectors, dem_with_replaced_targets
...
class CompiledPymatchingGapSampler(sinter.CompiledSampler):
    def __init__(self, task: sinter.Task, decoder: sinter.Decoder | None):
        ...
        dem_obs2det = dem_with_replaced_targets(
            dem,
            {
                stim.target_logical_observable_id(k): stim.target_relative_detector_id(
                    num_dets + k
                )
                for k in range(self.num_obs)
            },
        ...
        dem.append(
            "detector",
            (),
            [stim.target_relative_detector_id(aligned_circuit.num_detectors - 1)],
        )
...
def _decode_weight_with_pymatching_with_better_error_message(
    ...
    raise ValueError(
        "pymatching failed to decode a batch of shots, but then succeeded on each shot individually?"
    )
