...
def test_from_dem_end_to_end_d3():
    c = cultiv.make_end2end_cultivation_circuit(
        dcolor=3, dsurface=7, basis="Y", r_growing=2, r_end=1, inject_style="unitary"
    )
    c = gen.NoiseModel.uniform_depolarizing(1e-3).noisy_circuit_skipping_mpp_boundaries(
        c
    )
    dem = c.detector_error_model()
    dec = DesaturationSampler().compiled_sampler_for_task(
        sinter.Task(circuit=c, detector_error_model=dem)
    )
...
def test_from_dem_end_to_end_d5():
    c = cultiv.make_end2end_cultivation_circuit(
        dcolor=5, dsurface=11, basis="Y", r_growing=4, r_end=1, inject_style="unitary"
    )
    c = gen.NoiseModel.uniform_depolarizing(1e-3).noisy_circuit_skipping_mpp_boundaries(
        c
    )
