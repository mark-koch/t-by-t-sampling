...
class _DemError:
    ...
    def from_error_instruction(instruction: stim.DemInstruction) -> "_DemError":
        ...
        return _DemError(
            p=p, det_set=frozenset(gen.xor_sorted(det_list)), has_obs=has_obs
        )
...
class CompiledChromobiusGapSampler(sinter.CompiledSampler):
    def __init__(self, task: sinter.Task):
        if task.detector_error_model.num_observables != 1:
            raise NotImplementedError(
                f"{task.detector_error_model.num_observables=} != 1"
            )
        ...
        self.gap_dem_base.append(
            "detector",
            [-9, -9, -9, obs_color_basis],
            [stim.target_relative_detector_id(self.obs_det)],
        )
        self.main_dem.append(
            "detector",
            [-9, -9, -9, obs_color_basis],
            [stim.target_relative_detector_id(self.obs_det)],
        )
    ...
    def decode_shot(self, shot: np.ndarray) -> tuple[bool, int]:
        ...
        _, weight0 = self.gap_decoder.predict_weighted_obs_flips_from_dets_bit_packed(
            shot
        )
        shot[-1] ^= np.uint8(1 << (self.obs_det % 8))
        _, weight1 = self.gap_decoder.predict_weighted_obs_flips_from_dets_bit_packed(
            shot
        )
        shot[-1] ^= np.uint8(1 << (self.obs_det % 8))
