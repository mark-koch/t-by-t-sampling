...
class CompiledTwirlInterceptSampler(sinter.CompiledSampler):
    ...
    def _t_twirl(self, qubits: list[int]):
        ...
        xy_twirl = np.zeros(
            (self.num_qubits, self.simulator.batch_size), dtype=np.bool_
        )

        # Only twirl targeted qubits.
        for q in qubits:
            xy_twirl[q, :] = np.random.randint(
                0, 2, self.simulator.batch_size, dtype=np.bool_
            )
    ...
    def _do_t_or_s_or_z(self, targets: list[stim.GateTarget]):
        if self.is_t_like:
            ...
            self.simulator.do(
                stim.CircuitInstruction("S", [t.qubit_value for t in targets])
            )
        elif self.is_z_like:
    ...
    def _sample_once(self, shots: int) -> sinter.AnonTaskStats:
        ...
        discard_mask = np.any(
            self.simulator.get_detector_flips(bit_packed=False), axis=0
        )
        error_mask = np.any(
            self.simulator.get_observable_flips(bit_packed=False), axis=0
        )
