...
def sinter_samplers() -> dict[str, sinter.Sampler]:
    return {
        ...
        "vec_intercept_t": VecInterceptSampler(
            turns=0.25, sweep_bit_randomization=False
        ),
        "vec_intercept_z": VecInterceptSampler(turns=1, sweep_bit_randomization=False),
        "vec_intercept_s": VecInterceptSampler(
            turns=0.5, sweep_bit_randomization=False
        ),
        "vec_intercept_t_twirl": VecInterceptSampler(
            turns=0.25, sweep_bit_randomization=False
        ),
        "vec_intercept_z_twirl": VecInterceptSampler(
            turns=1, sweep_bit_randomization=False
        ),
        "vec_intercept_s_twirl": VecInterceptSampler(
            turns=0.5, sweep_bit_randomization=False
        ),
