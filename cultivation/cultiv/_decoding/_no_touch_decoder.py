...
class _GlobalLookup:
    ...
    def from_dem(dem: stim.DetectorErrorModel) -> "_GlobalLookup":
        ...
        return _GlobalLookup(
            det_to_local_lookup=det_to_local_lookup, obs_masks=obs_masks
        )
    ...
    def decode_shots_bit_packed(
        ...
        *,
        ...
        for k in range(len(dets)):
            prediction = self.decode_det_set(
                frozenset(np.flatnonzero(np.unpackbits(dets[k], bitorder="little")))
            )
