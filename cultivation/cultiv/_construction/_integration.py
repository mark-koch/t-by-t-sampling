...
from ._color_code import (
    make_color_code,
    make_color_code_grow_chunk,
    make_chunk_color_code_superdense_cycle,
)
from ._cultivation_stage import (
...
def make_escape_to_big_matchable_code_circuit(
    *,
    ...
    layer_circuit = gen.LayerCircuit.from_stim_circuit(
...
def make_end2end_cultivation_circuit(
    *,
    ...
    c = gen.LayerCircuit.from_stim_circuit(
        gen.compile_chunks_into_circuit(
            chunks,
            add_mpp_boundaries=True,
            flow_to_extra_coords_func=flow_to_extra_coords,
    ...
    c = c.with_whole_layers_slid_as_to_merge_with_previous_layer_of_same_type(
        gen.ResetLayer
    )
    c = c.with_whole_layers_slid_as_early_as_possible_for_merge_with_same_layer(
        gen.InteractLayer
    )
    c = c.with_locally_optimized_layers()
...
def make_escape_to_big_color_code_circuit(
    *, start_width: int, end_width: int, rounds: int, basis: Literal["X", "Y", "Z"]
) -> stim.Circuit:
    chunks = [
        make_color_code_grow_chunk(start_width, end_width, basis=basis),
        make_chunk_color_code_superdense_cycle(
            make_color_code(end_width), obs_basis=basis
        ).time_reversed()
        * rounds,
    ]
...
def make_surface_code_memory_circuit(
    *, dsurface: int, rounds: int, basis: Literal["X", "Y", "Z"]
) -> stim.Circuit:
    code = make_surface_code(width=dsurface, height=dsurface)
    init = (
        code.with_observables_from_basis("Y").mpp_init_chunk()
        if basis == "Y"
        else code.transversal_init_chunk(basis=basis)
    )
    chunks = [
        init,
