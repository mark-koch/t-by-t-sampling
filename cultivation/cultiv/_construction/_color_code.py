...
def make_color_code(
    base_width: int,
    *,
    obs_location: Literal[
        "all", "top", "bottom-left", "bottom-right", "x-top-z-bottom-right"
    ] = "bottom-left",
) -> gen.StabilizerCode:
    tiles = []
...
def make_growing_color_code_bell_pair_patch(
    start_base_width: int, end_base_width: int
) -> gen.Patch:
...
def make_color_code_grow_chunk(
    start_base_width: int, end_base_width: int, basis: Literal["X", "Y", "Z"]
) -> gen.Chunk:
    grow_patch = make_growing_color_code_bell_pair_patch(
        start_base_width, end_base_width
    )
