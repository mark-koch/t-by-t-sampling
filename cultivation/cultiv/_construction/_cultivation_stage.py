...
from ._injection_stage import (
    make_chunk_d3_init_unitary,
    make_chunk_d3_init_degenerate_teleport,
...
def make_chunk_d3_to_d5_color_code() -> gen.Chunk:
    ...
    included_out = {flow.end for flow in chunk.flows if flow.end}
    discarded_out = {
        tile.to_data_pauli_string() for tile in make_color_code(5).tiles
    } - included_out
...
def make_inject_and_cultivate_chunks_d3(
    *, style: Literal["degenerate", "bell", "unitary"]
) -> list[gen.Chunk | gen.ChunkReflow]:
    if style == "degenerate":
        return [
            make_chunk_d3_init_degenerate_teleport().with_obs_flows_as_det_flows(),
            make_chunk_color_code_superdense_cycle(
                make_color_code(3, obs_location="all"), obs_basis="Y"
            )
            .with_obs_flows_as_det_flows()
            .with_flag_added_to_all_flows("stage=cultivation"),
...
def make_inject_and_cultivate_chunks_d5(
    *, style: Literal["degenerate", "bell", "unitary"]
) -> list[gen.Chunk | gen.ChunkReflow]:
    return [
        *[
            chunk.with_obs_flows_as_det_flows()
            for chunk in make_inject_and_cultivate_chunks_d3(style=style)
        ],
        make_chunk_d3_to_d5_color_code().with_obs_flows_as_det_flows(),
        make_chunk_color_code_superdense_cycle(
            make_color_code(5, obs_location="all"), obs_basis="Y"
        )
        .time_reversed()
        .with_obs_flows_as_det_flows()
        .with_flag_added_to_all_flows("stage=cultivation")
