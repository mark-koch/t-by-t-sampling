...
def make_hybrid_color_surface_code(
    *, dcolor: int, dsurface: int, obs_location: Literal["left", "right", "transition"]
) -> gen.StabilizerCode:
    ...
    for tile in color_code.tiles:
        if tile.basis == "X" and len(tile.data_qubits) == 4 and "color=b" in tile.flags:
            ...
            tiles.append(
                tile.with_edits(
                    data_qubits=[m + d for d in [-1, 1j, 1 + 1j, 2, -1j, 1 - 1j]]
                )
            )
            ...
            tile.basis == "Z" and len(tile.data_qubits) == 4 and "color=r" in tile.flags
        ):
            ...
            tiles.append(
                tile.with_edits(
                    data_qubits=[m + d for d in [-1, 1j, 1 + 1j, 2, 2 - 1j, -1j]]
                )
            )
    ...
    bottom_left_obs_x_qubits |= (
        surface_code.get_observable_by_basis(0, "X").keys() - used_qubits
    )
    ...
    if obs_location == "right":
        ...
        obs_z_qubits = (
            color_code.get_observable_by_basis(0, "Z").keys()
            | surface_code.get_observable_by_basis(0, "Z").keys()
        )
    elif obs_location == "transition":
        prev_code = make_color_code(
            base_width=dcolor, obs_location="x-top-z-bottom-right"
        )
        obs_x_qubits = prev_code.get_observable_by_basis(0, "X").keys() | {
            q for q in patch.data_set if q.imag == 0 and q.real > dcolor * 2 - 2
        }
        obs_z_qubits = prev_code.get_observable_by_basis(0, "Z").keys() | {
            q for q in patch.data_set if q.imag == -1 and q.real > dcolor * 2 - 2
        }
    else:
...
def make_color_code_grown_into_surface_code_then_ablated_into_matchable_code_simple(
    *,
    ...
    grown_code = make_hybrid_color_surface_code(
        dcolor=dcolor, dsurface=dsurface, obs_location="left"
    )
...
def make_color_code_all_to_transition_reflow(
    code1: gen.StabilizerCode, code2: gen.StabilizerCode
) -> gen.ChunkReflow:
    ...
    out2in[end] = [
        ...
        *[
            ...
            for tile in code1.tiles
            if ("color=g" in tile.flags and "basis=X" in tile.flags)
            or ("color=r" in tile.flags and "basis=Z" in tile.flags)
        ],
    ]
...
def make_color_code_to_big_matchable_code_escape_chunks(
    *,
    ...
    code2a = make_hybrid_color_surface_code(
        dcolor=dcolor, dsurface=dsurface, obs_location="transition"
    )
    code2b = make_hybrid_color_surface_code(
        dcolor=dcolor, dsurface=dsurface, obs_location="right"
    )
    ...
    return [
        ...
        make_color_code_to_growing_code_chunk(
            start_code=code1b, end_code=code2a, obs_basis=basis
        ),
...
def make_color_code_grown_into_surface_code_then_ablated_into_matchable_code_full_edges(
    *,
    ...
    for tile in grown_code.tiles:
        if (
            "color=g" in tile.flags
            and "basis=X" in tile.flags
            and len(tile.data_set) in [4, 6]
        ):
            m = tile.measure_qubit
    ...
    return grown_code.with_edits(
        ...
        logicals=[
            (
                grown_code.logicals[0][0],
                gen.PauliMap(zs=grown_code.logicals[0][1].keys() - remove_z),
            ),
        ],
    )
    ...
    *,
    ...
    for tile in grown_code.tiles:
        ...
        if m in near_left_boundary_greens:
            ...
            if (
                m - 4 in left_boundary_greens
                and len(grown_code.stabilizers.m2tile[m - 3j - 2].data_set) != 5
            ):
                extra_flags.clear()
            tiles.append(
                tile.with_edits(
            ...
            tiles.append(
                tile.with_edits(
                    flags=tile.flags ^ {"cycle=bell-flagged-Z", "cycle=superdense"}
                )
            )
            ...
            tiles.append(
                tile.with_edits(
                    flags=tile.flags ^ {"cycle=bell-flagged-X", "cycle=superdense"}
                )
            )
        else:
            tiles.append(tile)
...
def make_color_code_to_growing_code_chunk(
    *,
    ...
    flows.extend(
        start_code.auto_obs_passthrough_flows(obs_basis=obs_basis, next_code=end_code)
    )
...
def make_hybrid_code_round_chunk(
    *,
    ...
    bell_pairs = [
        ...
        if tile.basis == "X"
        if "cycle=superdense" in tile.flags
        or "cycle=bell-flagged-X" in tile.flags
        or "cycle=bell-flagged-Z" in tile.flags
    ]
    ...
    for tile in code.stabilizers.tiles:
        ...
        m = tile.measure_qubit
        if tile.basis == "X" and (
            "cycle=superdense" in tile.flags or "cycle=bell-flagged-X" in tile.flags
        ):
            tile_cx(cx2, m, m - 1)
            tile_cx(cx2, m + 1, m + 2)
            tile_cx(cx3, m, m + 1j)
            ...
            tile_cx(
                cx1,
                m,
                m - 1j if m - 1j in tile.data_set else m + 1j,
                rev=tile.basis == "Z",
            )
            tile_cx(
                cx2,
                m,
                m - 1 if m - 1 in tile.data_set else m + 1,
                rev=tile.basis == "Z",
            )
            ...
            cx3.append((m, m - 1j))
            cx4.append((m, m - 1))
        elif tile.basis == "Z" and (
            "cycle=superdense" in tile.flags or "cycle=bell-flagged-Z" in tile.flags
        ):
            m -= 1
    ...
    flows.extend(
        prev_code.auto_obs_passthrough_flows(next_code=code, obs_basis=obs_basis)
    )
