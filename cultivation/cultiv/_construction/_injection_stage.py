        ...
        if inst.name == "S":
            new_circuit.append(
                stim.CircuitInstruction(
                    forward, inst.targets_copy(), inst.gate_args_copy()
                )
            )
        elif inst.name == "S_DAG":
            new_circuit.append(
                stim.CircuitInstruction(
                    backward, inst.targets_copy(), inst.gate_args_copy()
                )
            )
        elif inst.name == "MPP":
                    ...
                    for e in part:
                        ts.append(
                            stim.target_pauli(
                                e.qubit_value,
                                basis,
                                invert=change_remaining and sign_change,
                            )
                        )
...
def injection_chunk_with_rewritten_injection_rotation(
    ...
    *,
    ...
    new_circuit = injection_circuit_with_rewritten_injection_rotation(
        chunk.circuit, turns
    )
        ...
        if (has_y_start and has_y_end) or (has_x_start and has_x_end):
            ...
            if flow.measurement_indices and not allow_feedback_z_stabilizer:
                if is_init_circuit:
                    ...
                    new_flows.append(
                        flow.with_edits(start=flow.end, measurement_indices=[])
                    )
        else:
            new_flows.append(flow)
