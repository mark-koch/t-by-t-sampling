    ...
    *,
    ...
    for tile in next_code.tiles:
        ...
        if any(gained_bases.get(q, b) != b for q, b in ps.items()):
            ...
            flows.append(
                gen.Flow(
                    start=gen.PauliMap(
                        {q: b for q, b in ps.items() if q not in gained_bases}
                    ),
                    end=ps,
    ...
    for k, obs in enumerate(next_code.logicals):
        flows.append(
            gen.Flow(
                start=gen.PauliMap(
                    {q: b for q, b in obs.items() if q not in gained_bases}
                ),
                end=obs,
                obs_key=k,
            )
            ...
            flows.append(
                gen.Flow(
                    start=ps,
                        ...
                        {q: b for q, b in ps.items() if q not in lost_qubits}
                    ),
                    center=tile.measure_qubit,
        ...
        flows.append(
            gen.Flow(
                start=obs,
                    ...
                    {q: b for q, b in obs.items() if q not in lost_qubits}
                ),
                obs_key=k,
...
def make_surface_code_cnot(
    *,
    ...
    circuit = gen.stim_circuit_with_transformed_coords(
        circuit, lambda e: e * (0.5 + 0.5j)
    )
