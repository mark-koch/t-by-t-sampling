...
from ._escape_stage import (
    ...
    make_color_code_all_to_transition_reflow,
)
...
def test_make_color_code_to_big_matchable_code_escape_chunks():
    chunks = make_color_code_to_big_matchable_code_escape_chunks(
        dcolor=5, dsurface=11, basis="Y", r_growing=3, r_end=2
    )
    for chunk in chunks:
        chunk.verify()
...
def test_make_hybrid_color_surface_code(dcolor: int, dsurface: int, obs_location: Any):
    code = make_hybrid_color_surface_code(
        dcolor=dcolor, dsurface=dsurface, obs_location=obs_location
    )
    code.verify()
...
def test_make_color_code_to_growing_code_chunk(dcolor: int, dsurface: int, basis: Any):
    ...
    code2 = make_hybrid_color_surface_code(
        dcolor=dcolor, dsurface=dsurface, obs_location="transition"
    )
    code3 = make_hybrid_color_surface_code(
...
@pytest.mark.parametrize("basis", ["X", "Z"])
@pytest.mark.parametrize("obs_location", ["left", "right", "transition"])
def test_make_hybrid_code_round_chunk(
    dcolor: int, dsurface: int, basis: Any, obs_location: Any
):
    code1 = make_hybrid_color_surface_code(
        dcolor=dcolor, dsurface=dsurface, obs_location=obs_location
    )
...
def test_make_color_code_grown_into_surface_code_then_ablated_into_matchable_code_simple(
    dcolor: int, dsurface: int
):
    c = make_color_code_grown_into_surface_code_then_ablated_into_matchable_code_simple(
        dcolor=dcolor, dsurface=dsurface, desaturate=True
    )
    assert (
        c.make_code_capacity_circuit(noise=1e-3).detector_error_model(
...
@pytest.mark.parametrize("dcolor,dsurface", [(3, 6), (3, 7), (5, 11), (7, 14)])
def test_make_color_code_grown_into_surface_code_then_ablated_into_matchable_code_full_edges(
    dcolor: int, dsurface: int
):
    c = make_color_code_grown_into_surface_code_then_ablated_into_matchable_code_full_edges(
        dcolor=dcolor, dsurface=dsurface, desaturate=True
    )
    assert (
        c.make_code_capacity_circuit(noise=1e-3).detector_error_model(
