...
@pytest.mark.parametrize(
    "obs_location",
    ["all", "top", "bottom-right", "bottom-left", "x-top-z-bottom-right"],
)
def test_make_color_code(d: int, obs_location: Any):
    ...
    for tile in code.tiles:
        if tile.basis == "Z":
            ...
            assert code.stabilizers.m2tile[tile.measure_qubit - 1] == tile.with_edits(
                bases="X",
                flags=tile.flags ^ {"basis=X", "basis=Z"},
                measure_qubit=tile.measure_qubit - 1,
            )
        else:
            assert tile.basis == "X"
...
def test_make_color_code_superdense_cycle(
    d: int, basis: Any, check_x: bool, check_z: bool
):
    code = make_color_code(d)
    chunk = make_chunk_color_code_superdense_cycle(
        code, obs_basis=basis, check_x=check_x, check_z=check_z
    )
    chunk.verify()
...
def test_x():
    ...
    patch = gen.Patch(
        ...
        + [
            ...
            if tile.basis == "Z"
            if tile.measure_qubit in before.measure_set
            or "color=b" in tile.flags
            or "color=g" in tile.flags
        ]
    )
    patch.write_svg(
        "tmp.svg",
        tile_color_func=lambda e: (
            (0.5, 1, 1) if len(e.data_set) == 2 else tile_rgb_color(e)
        ),
        show_coords=False,
    )
