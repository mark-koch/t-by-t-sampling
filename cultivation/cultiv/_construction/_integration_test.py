...
def test_make_escape_to_big_matchable_code_circuit(
    dcolor: int, dsurface: int, basis: Any
):
    ...
    circuit = gen.NoiseModel.uniform_depolarizing(
        1e-3
...
def test_make_matchable_idle_circuit(
    dcolor: int, dsurface: int, basis: Any, rounds: int
):
    circuit = make_idle_matchable_code_circuit(
        dcolor=dcolor, dsurface=dsurface, basis=basis, rounds=rounds
    )
    circuit = gen.NoiseModel.uniform_depolarizing(
        1e-3
    ).noisy_circuit_skipping_mpp_boundaries(circuit)
...
def test_make_matchable_idle_circuit_exact():
    assert make_idle_matchable_code_circuit(
        dcolor=3, dsurface=6, basis="Y", rounds=100
    ) == stim.Circuit("""
        QUBIT_COORDS(0, -1) 0
        QUBIT_COORDS(0, 0) 1
        QUBIT_COORDS(1, -1) 2
...
def test_make_growth_only_circuit_8(basis: Any):
    ...
    noisy_circuit = gen.NoiseModel.uniform_depolarizing(
        1e-3
    ).noisy_circuit_skipping_mpp_boundaries(circuit)
    ...
    if basis == "X" or basis == "Z":
        ...
        decoder = CompiledChromobiusGapSampler(
            sinter.Task(
