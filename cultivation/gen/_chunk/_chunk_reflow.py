...
class ChunkReflow:
    ...
    def from_auto_rewrite(
        *,
        ...
        out2in: dict[
            PauliMap | KeyedPauliMap, list[PauliMap | KeyedPauliMap] | Literal["auto"]
        ],
    ):
        new_out2in = {}
        unsolved = []
    ...
    def with_obs_flows_as_det_flows(self):
        return ChunkReflow(
            out2in={
                PauliMap(k): [PauliMap(v) for v in vs] for k, vs in self.out2in.items()
            },
    ...
    def __repr__(self) -> str:
        return (
            f"gen.ChunkReflow(out2in={self.out2in!r}, discard_in={self.discard_in!r})"
        )
