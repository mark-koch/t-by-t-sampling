...
def test_swap_cancellation():
    ...
    ).with_locally_optimized_layers() == LayerCircuit.from_stim_circuit(
        stim.Circuit(
            """
        SWAP 0 1 4 5 7 8
        TICK
...
def test_with_rotation_layers_moved_earlier():
    ...
    ).with_whole_rotation_layers_slid_earlier() == LayerCircuit.from_stim_circuit(
        stim.Circuit(
            """
        CX 0 1
        TICK
...
def test_with_whole_measurement_layers_slid_earlier():
    ...
    ).with_whole_measurement_layers_slid_earlier() == LayerCircuit.from_stim_circuit(
        stim.Circuit(
            """
        M 0 1 2 3
        CX rec[-4] 5
...
def test_with_whole_layers_slid_as_early_as_possible_for_merge_with_same_layer():
    ...
    ).with_whole_layers_slid_as_to_merge_with_previous_layer_of_same_type(
        ResetLayer
